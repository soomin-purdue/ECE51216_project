...
def format_assignment(assignment: Assignment, variables: Iterable[int]) -> str:
    ...
    for variable in sorted(set(variables)):
        if variable in assignment:
            output.append(str(variable if assignment[variable] else -variable))
    output.append("0")
    return " ".join(output)
...
def build_cli() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--print-model",
        action="store_true",
        help="Print the satisfying assignment in DIMACS style when SAT",
    )
    return parser
...
def main() -> int:
    ...
    for raw_path in args.inputs:
        ...
        variables = collect_variables(formula) or set(range(1, num_vars + 1))
        ...
        if args.print_model and result.is_sat:
            print(f"MODEL: {format_assignment(result.assignment, variables)}")
        print()
